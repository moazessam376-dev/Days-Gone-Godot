...
# WeaponManager (runtime switching). Order matters — index 0 is the weapon
# the saved scene ships equipped, matching the autoplayed pistol idle.
const WEAPON_RES := [
...
func _attach_weapon(skel: Skeleton3D) -> void:
	...
	for path: String in WEAPON_RES:
		...
		if w == null:
			...
			return
		weapons.append(w)
	var equipped: WeaponResource = weapons[0]
